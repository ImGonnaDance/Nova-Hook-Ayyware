...
public:
	...
	Vector GetBonePos(int i)
	{
		...
		if (this->SetupBones(boneMatrix, 128, BONE_USED_BY_HITBOX, GetTickCount64()) != NULL)
		{
	...
	int getChockedTicks()
	{
		if (GetSimulationTime() > getOldSimulTime())
			return TIME_TO_TICKS(fabsf(GetSimulationTime() - getOldSimulTime()));
		return 0;
	}
	...
	//void SetAbsOrigin(const Vector& ori) {
	//	((void(__thiscall*)(IClientEntity*, const Vector&))offsets->SetAbsOriginEx)(this, ori);
	//}

	//CBaseCombatWeapon* GetWeapon() {
	//	uintptr_t weaponData = *(uintptr_t*)((uintptr_t)this + 0x2EE8);
	//	return (CBaseCombatWeapon*)Interfaces::EntList->GetClientEntityFromHandle(weaponData);
	//}
